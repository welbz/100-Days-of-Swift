...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let savedPeople = defaults.object(forKey: "people") as? Data {
            if let decodedPeople = try?
                NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedPeople) as? [Person] { // typecast optionlly as a Person array
                people = decodedPeople // get back full unarchived array
            }
        }
    }
    ...
    // the archivedData() method of NSKeyedArchiver turns an object graph into a Data object using those NSCoding methods we just added to our class. Because we make changes to the array by adding people or by renaming them, let's create a single save() method we can use anywhere that's needed:
    func save() {
        if let savedData = try? NSKeyedArchiver.archivedData(withRootObject: people, requiringSecureCoding: false) {
            let defaults = UserDefaults.standard
            defaults.set(savedData, forKey: "people")
        }
        // writes our people array to the people key but it will first wrap that inside data ojbect called savedDate
    }
}
